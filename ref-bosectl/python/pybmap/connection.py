...
class BmapConnection:
    ...
    def auto_off(self):
        """Auto-off standby timeout in minutes (int; 0 == Never)."""
        return self._get("auto_off")
    ...
    def connected_device(self):
        """Bluetooth MAC of the currently connected source device [4.9]."""
        return self._get("connected_device")

    def paired_devices(self):
        """List of remembered/paired device MACs [4.4] (read-only)."""
        return self._get("paired_devices")
    ...
    def modes(self):
        ...
        Preferred path is GetAllModes [31.1] START/GET which streams every
        slot's STATUS in one drain (QC Ultra 2). Devices where that is
        auth-gated or silent (QC Ultra 1st gen) fall back to enumerating each
        slot in MODE_SLOTS with an individual ModeConfig GET.
        """
        mode_config_addr = self._feature("mode_config")["addr"]
        parser = self._feature("mode_config").get("parser")
        modes = {}

        if self.has_feature("get_all_modes"):
            self._collect_modes(
                self._start_drain("get_all_modes"),
                mode_config_addr, parser, modes,
            )

        if not modes:
            for idx in getattr(self._device, "MODE_SLOTS", []):
                # Each per-slot GET returns exactly one ModeConfig STATUS, so
                # drain=False (return on first reply) is safe and ~10x faster
                # than waiting out the inter-packet drain gap on every slot.
                data = self._transport.send_recv(
                    bmap_packet(mode_config_addr[0], mode_config_addr[1],
                                OP_GET, bytes([idx])),
                    drain=False,
                )
                self._collect_modes(
                    parse_all_responses(data), mode_config_addr, parser, modes,
                )
        return modes

    def mode_config(self, idx, drain=True):
        """Read a single mode slot's ModeConfig via one GET (cheap).

        Avoids enumerating all MODE_SLOTS when only the active mode's config
        is needed (e.g. for a fast status refresh after a mode switch).
        Returns a ModeConfig or None.
        """
        addr = self._feature("mode_config")["addr"]
        parser = self._feature("mode_config").get("parser")
        data = self._transport.send_recv(
            bmap_packet(addr[0], addr[1], OP_GET, bytes([idx])),
            drain=drain,
        )
        modes = {}
        self._collect_modes(parse_all_responses(data), addr, parser, modes)
        return modes.get(idx)

    @staticmethod
    def _collect_modes(responses, addr, parser, modes):
        """Pick ModeConfig STATUS packets out of a response list into modes."""
        for resp in responses:
            if (resp.fblock == addr[0] and resp.func == addr[1] and
    ...
    def set_mode_idx(self, idx, announce=False):
        """Switch to a mode by slot index [31.3] START.

        Unlike set_mode(name), this is unambiguous when several custom slots
        share the stored name "None" (gen-1 stores custom names only in the
        prompt bytes, so name-based lookup can collide)."""
        resp = self._start("current_mode", bytes([idx, 1 if announce else 0]))
        if resp and resp.op != OP_RESULT:
            raise BmapDeviceError("Mode switch failed: %s" % fmt_response(resp))
    ...
    def set_cnc(self, level):
        ...
        10 = most ambient pass-through. On gen-1 this writes the current mode's
        ModeConfig [31.6] with the Wind Block trigger byte forced off, so the
        level actually persists (sending Wind Block=on would clamp it to max).
        """
        if not 0 <= level <= 10:
    ...
    def _update_audio_settings(self, **overrides):
        ...
        Preferred path uses AudioModesSettingsConfig [31.10] SETGET — the same
        register the Bose app writes on QC Ultra 2+. Devices without [31.10]
        (QC Ultra 1st gen returns FuncNotSupp) fall back to editing the current
        mode's ModeConfig [31.6], which carries the same CNC/spatial/wind/anc
        fields per-slot.
        """
        ...
        if self.has_feature("mode_config") and self.has_feature("current_mode"):
            self._update_current_mode_settings(**overrides)
            return

        raise BmapError("Device does not support direct audio settings")

    def _update_current_mode_settings(self, **overrides):
        """Fallback audio-settings write via the current mode's ModeConfig.

        Reads the active mode, applies overrides to its CNC/spatial/wind/anc
        fields, and writes it back with a ModeConfig SETGET. Only editable
        modes (custom profiles) can be changed; locked presets raise.
        """
        idx = self.mode_idx()
        if idx is None:
            raise BmapError("Could not read current mode index")
        config = self.modes().get(idx)
        if config is None:
            raise BmapError("Current mode %d config not readable" % idx)
        if not config.editable:
            raise BmapError(
                "Current mode '%s' is a locked preset; switch to an editable "
                "mode (custom profile) before changing noise settings" % config.name
            )
        self._write_mode_from_config(idx, config, **overrides)
    ...
    def set_auto_off(self, minutes):
        """Set the auto-off standby timer.

        Args:
            minutes: int minutes (0 == Never) or a label string
                     (never/5min/20min/40min/1h/3h/24h).
        """
        feat = self._feature("auto_off")
        builder = feat.get("builder")
        self._setget("auto_off", builder(minutes))

    def set_prompts(self, enabled):
        """Toggle voice prompts. Preserves current language setting.

        Not available where the voice-prompt block is read-only (e.g. QC Ultra
        1st gen, whose [1.3] write format is not yet mapped)."""
        feat = self._feature("voice_prompts")
        builder = feat.get("builder")
        if builder is None:
            raise BmapError(
                "%s: voice-prompt toggling is not supported (read-only)"
                % self._device.DEVICE_INFO.get("name", "Device")
            )
    ...
    def _write_mode_from_config(self, slot, config, **overrides):
        ...
        # Gen-1 ModeConfig [31.6] quirk: the transmitted `wind_block` byte is a
        # command TRIGGER, not persisted state. Sending wind_block=1 makes the
        # firmware clamp cnc_level->0 and anc->on (Wind Block == max ANC, which
        # matches the app's "Wind Block automatically adjusts noise
        # cancellation"). The STATUS readback of that byte is meaningless (reads
        # 1 on every slot regardless of the app toggle), so we must NOT carry it
        # back into a write. Default the trigger to 0 so cnc/spatial/anc edits
        # actually stick; only send wind_block=1 when the caller explicitly asks
        # to engage Wind Block.
        if getattr(self._device, "MODE_WIND_IS_TRIGGER", False):
            wind_value = 1 if overrides.get("wind_block", False) else 0
        else:
            wind_value = overrides.get("wind_block", config.wind_block)
